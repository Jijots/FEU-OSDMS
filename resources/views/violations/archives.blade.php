        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 border-b-2 border-slate-300 pb-5">
            <div>
                <div class="flex items-center gap-3 mb-1">
                    <a href="{{ route('violations.report') }}" class="text-slate-400 hover:text-indigo-600 transition-colors">
                ...
                <p class="text-sm text-slate-400 mt-1 font-bold uppercase tracking-widest ml-9">Historical Disciplinary Records</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden opacity-90">
            <div class="bg-slate-50 border-b border-slate-200 px-6 py-3">
                <p class="text-xs font-bold text-slate-500 uppercase tracking-widest flex items-center gap-2">
                    ...
                    Warning: Force Deleting records here will permanently erase the student's disciplinary history for this incident.
                </p>
            </div>
                ...
                <table class="w-full text-left text-sm whitespace-nowrap">
                    <thead class="bg-slate-100 text-slate-500 font-extrabold uppercase tracking-wider text-[11px]">
                        <tr>
                            ...
                            <th class="p-5 border-b border-slate-200">Offense Type</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="hover:bg-slate-50 transition-colors">
                            <td class="p-5 font-semibold text-red-600">{{ \Carbon\Carbon::parse($violation->deleted_at)->format('M d, Y - h:i A') }}</td>

                            <td class="p-5">
                                @if($violation->student)
                                    <p class="font-bold text-slate-700 text-base">{{ $violation->student->name }}</p>
                                    <p class="text-xs font-semibold text-slate-400 mt-0.5">{{ $violation->student->id_number }}</p>
                                @else
                                    <span class="text-slate-400 font-bold text-xs uppercase tracking-wider font-mono">ID: {{ $violation->student_id }}</span>
                                @endif
                            </td>
                            ...
                            <td class="p-5 text-right space-x-3">
                                <form action="{{ route('violations.restore', $violation->id) }}" method="POST" class="inline-block">
                                    @csrf
                                ...
                                <form action="{{ route('violations.force-delete', $violation->id) }}" method="POST" class="inline-block" onsubmit="return confirm('CRITICAL WARNING: Are you sure you want to permanently purge this disciplinary record? This action cannot be undone.');">
                                    @csrf
                                    @method('DELETE')
                        ...
                        <tr>
                            <td colspan="4" class="p-12 text-center">
                                ...
                                <p class="text-sm text-slate-400 mt-1">No disciplinary archives found.</p>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
