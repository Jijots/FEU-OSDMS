<x-app-layout>
    <div class="max-w-4xl mx-auto px-8 lg:px-12 py-10">
        ...
        <div class="mb-10 flex items-center justify-between border-b-2 border-slate-100 pb-6">
            ...
            <span class="px-5 py-2.5 bg-slate-100 text-slate-600 font-bold rounded-xl text-sm border-2 border-slate-200 uppercase tracking-wide">New Entry</span>
        </div>
        ...
        <form action="{{ route('assets.store') }}" method="POST" enctype="multipart/form-data" x-data="{ photoPreview: null }" class="space-y-8 bg-white border-2 border-slate-200 rounded-2xl p-10 shadow-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                    ...
                    <input type="text" name="item_name" class="w-full bg-slate-50 border-2 border-slate-200 rounded-xl p-3.5 text-base font-semibold text-slate-800 focus:border-[#004d32] focus:ring-0 transition-colors" placeholder="e.g. iPhone 13 Pro Max" required>
                </div>

                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-2">Location Logged</label>
                    <input type="text" name="location_found" class="w-full bg-slate-50 border-2 border-slate-200 rounded-xl p-3.5 text-base font-semibold text-slate-800 focus:border-[#004d32] focus:ring-0 transition-colors" placeholder="e.g. T404, Tech Building Lobby" required>
                </div>
            </div>

            <div>
                <label class="block text-sm font-bold text-slate-700 mb-2">Detailed Description</label>
                <textarea name="description" rows="4" class="w-full bg-slate-50 border-2 border-slate-200 rounded-xl p-4 text-base font-medium text-slate-800 focus:border-[#004d32] focus:ring-0 placeholder:font-normal resize-none transition-colors" placeholder="Provide distinct characteristics like color, brand, or unique markings..." required></textarea>
            </div>
            ...
            <div class="border-t-2 border-slate-100 pt-8">
                ...
                <div class="flex items-start gap-4 p-5 bg-blue-50 border-2 border-blue-200 rounded-xl cursor-pointer hover:border-blue-300 transition-all shadow-sm" onclick="document.getElementById('is_stock_image').click();">
                    <div class="flex items-center h-6 shrink-0 mt-0.5">
                        <input type="checkbox" name="is_stock_image" id="is_stock_image" value="1" onclick="event.stopPropagation()" class="w-6 h-6 text-[#004d32] border-2 border-blue-300 rounded focus:ring-[#004d32] transition-all cursor-pointer bg-white">
                    </div>
                    <div>
                        ...
                        <p class="text-sm font-medium text-blue-700 mt-1 leading-relaxed">Check this box if the photo provided is a visual reference downloaded online and not a live capture of the actual item. This tells the AI to match general shape and color.</p>
                    </div>
                </div>
            </div>
