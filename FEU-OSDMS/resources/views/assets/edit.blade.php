<x-app-layout>
    ...
    <div class="max-w-2xl mx-auto py-10">
        <form action="{{ route('assets.update', $item->id) }}" method="POST" enctype="multipart/form-data" class="bg-white p-10 rounded-3xl shadow-sm border border-gray-100" x-data="{ photoPreview: null }">
            @csrf @method('PUT')
            <div class="space-y-6">
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="date" name="date_lost" value="{{ $item->date_lost }}" class="w-full px-6 py-4 rounded-2xl border-gray-200 bg-gray-50 font-bold" required>
                    </div>
                </div>
                <div>
                ...
                <div class="flex items-center gap-3 p-5 bg-blue-50/50 rounded-2xl border border-blue-100 transition-all hover:bg-blue-50">
                    <input type="checkbox" name="is_stock_image" id="is_stock_image" class="w-5 h-5 rounded text-feu-green" value="1" {{ $item->is_stock_image ? 'checked' : '' }}>
                ...
                <div>
                    ...
                    <div class="border-2 border-dashed border-gray-200 rounded-3xl p-6 text-center">
                        <template x-if="photoPreview"><img :src="photoPreview" class="h-48 mx-auto rounded-xl object-contain mb-4"></template>
                        ...
                        <button type="button" @click="$refs.photo.click()" class="bg-white border px-4 py-2 rounded-lg font-bold">Replace Image</button>
                    </div>
                </div>

                <div class="flex gap-4 pt-4">
                    <a href="{{ route('assets.index') }}" class="flex-1 text-center py-4 font-bold text-gray-400">Cancel</a>
                    <button type="submit" class="flex-[2] py-4 bg-feu-green text-white font-black rounded-2xl">Save Changes</button>
                </div>
            </div>
        </form>
    </div>
