        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 border-b-2 border-slate-300 pb-5">
            <div>
                <div class="flex items-center gap-3 mb-1">
                    <a href="{{ route('gate.index') }}" class="text-slate-400 hover:text-indigo-600 transition-colors">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
                    </a>
                    <h1 class="text-3xl font-extrabold text-slate-500 tracking-tight flex items-center gap-3">
                        <svg class="w-8 h-8 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"></path></svg>
                        Archived Gate Logs
                    </h1>
                </div>
                <p class="text-sm text-slate-400 mt-1 font-bold uppercase tracking-widest ml-9">Deleted & Hidden Entry Records</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden opacity-90">
            <div class="bg-slate-50 border-b border-slate-200 px-6 py-3">
                <p class="text-xs font-bold text-slate-500 uppercase tracking-widest flex items-center gap-2">
                    ...
                    Warning: Force Deleting records here will permanently erase them from the database.
                </p>
            </div>
                ...
                <table class="w-full text-left text-sm whitespace-nowrap">
                    <thead class="bg-slate-100 text-slate-500 font-extrabold uppercase tracking-wider text-[11px]">
                        <tr>
                            ...
                            <th class="p-5 border-b border-slate-200">Entry Reason</th>
                            <th class="p-5 border-b border-slate-200">Original Time In</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="hover:bg-slate-50 transition-colors">
                            <td class="p-5 font-semibold text-red-600">{{ \Carbon\Carbon::parse($entry->deleted_at)->format('M d, Y - h:i A') }}</td>

                            <td class="p-5">
                                @if($entry->student)
                                    <p class="font-bold text-slate-700">{{ $entry->student->name }}</p>
                                    <p class="text-xs font-semibold text-slate-400 mt-0.5">{{ $entry->student->id_number }}</p>
                                @else
                            ...
                            <td class="p-5">
                                <span class="inline-flex px-3 py-1 rounded-lg text-xs font-bold uppercase bg-slate-100 text-slate-600 border border-slate-200">{{ $entry->reason }}</span>
                            </td>

                            <td class="p-5 font-mono text-xs font-bold text-slate-500">
                                {{ \Carbon\Carbon::parse($entry->time_in)->format('M d, Y - h:i A') }}
                            </td>

                            <td class="p-5 text-right space-x-3">
                                <form action="{{ route('gate.restore', $entry->id) }}" method="POST" class="inline-block">
                                    @csrf
                                ...
                                <form action="{{ route('gate.force-delete', $entry->id) }}" method="POST" class="inline-block" onsubmit="return confirm('CRITICAL WARNING: Are you sure you want to permanently delete this gate log? This action cannot be undone.');">
                                    @csrf
                                    @method('DELETE')
